    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        def train(lr, cliprange, obs, returns, masks, actions, values, neglogpacs, states=None):
            if not hier:
                ...
                obs_by_sketch = unmap(obs)
                for kw in obs_by_sketch:
                    with tf.variable_scope(kw):
                        obs = obs_by_sketch[kw]
                        advs = returns - values
                        advs = (advs - advs.mean()) / (advs.std() + 1e-8)
                        td_map = {train_model.X:obs, A:actions, ADV:advs, R:returns, LR:lr,
                                CLIPRANGE:cliprange, OLDNEGLOGPAC:neglogpacs, OLDVPRED:values}
...
class Runner(AbstractEnvRunner):
    def __init__(self, *, env, model, nsteps, gamma, lam, hier=False):
        if hier: #override parent init (not modular)
            self.env = env
            tokens, box = env.observation_space.spaces
            self.model = model
            nenv = env.num_envs
            ...
            (self.tokens[:], self.obs[:]) = env.reset()
            self.nsteps = nsteps
    ...
    def run(self):
        ...
        if self.hier: mb_tokens = np.asarray(mb_tokens, dtype = self.tokens.dtype)
...
class HierModel(object):
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        def train(lr, cliprange, obs, returns, masks, actions, values, neglogpacs, states=None):
            obs_by_sketch = unmap(obs, (returns, masks, actions, values, neglogpacs, states))
            for kw in obs_by_sketch:
                with tf.variable_scope(kw):
                    obs, returns, masks, actions, values, neglogpacs = obs_by_sketch[kw]
                    advs = returns - values
                    advs = (advs - advs.mean()) / (advs.std() + 1e-8)
                    td_map = {train_model.X:obs, A:actions, ADV:advs, R:returns, LR:lr,
                            CLIPRANGE:cliprange, OLDNEGLOGPAC:neglogpacs, OLDVPRED:values}
        ...
        self.train_model = train_model
        self.act_model = act_model
        self.step = act_model.hier_step
        self.value = act_model.hier_value
        self.initial_state = act_model.initial_state
        self.save = save
        self.load = load
        tf.global_variables_initializer().run(session=sess) #pylint: disable=E1101

class HierRunner(AbstractEnvRunner):
...
def learn(*, policy, env, nsteps, total_timesteps, ent_coef, lr,
    ...
    for update in range(1, nupdates+1):
        ...
        if hier: (token, obs), returns, masks, actions, values, neglogpacs, states, epinfos = runner.run() #pylint: disable=E0632
        else: obs, returns, masks, actions, values, neglogpacs, states, epinfos = runner.run()
        ...
        if states is None: # nonrecurrent version
            ...
            for _ in range(noptepochs):
                ...
                for start in range(0, nbatch, nbatch_train):
                    ...
                    if hier:
                        ob_slices = (arr[mbinds] for arr in (token, obs))
                        mblossvals.append(model.train(lrnow, cliprangenow, ob_slices, *slices))
                    else: 
